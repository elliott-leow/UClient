...
public class LowBlockWarning extends Module {
	...
	public LowBlockWarning() {
		super("Low Block Warning", "Tells you when you are low on blocks", Category.HUD);
	...
	if(event.type == RenderGameOverlayEvent.ElementType.TEXT && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem() != null && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem().getItem() instanceof ItemBlock && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem().stackSize < (int)Kabbalah.settingsManager.getSettingByName(this, "Min Blocks").getValDouble()) {
		...
		if (Kabbalah.settingsManager.getSettingByName(this, "Only Clay").getValBoolean() && Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem().getItem() == Item.getItemFromBlock(Blocks.stained_hardened_clay)) {
			GL11.glPushMatrix();
			if (Minecraft.getMinecraft().thePlayer.getHeldItem().stackSize>1) {
				fr.drawStringWithShadow(Minecraft.getMinecraft().thePlayer.getHeldItem().stackSize + " blocks remaining", (int) (sr.getScaledWidth()*(Kabbalah.settingsManager.getSettingByName(this, "Width").getValDouble()/100) - 45), (int) (sr.getScaledHeight()* (Kabbalah.settingsManager.getSettingByName(this, "Height").getValDouble()/100)), 0xff82F6FF);
			}
			else {
				fr.drawStringWithShadow(Minecraft.getMinecraft().thePlayer.getHeldItem().stackSize + " block remaining", (int) (sr.getScaledWidth()*(Kabbalah.settingsManager.getSettingByName(this, "Width").getValDouble()/100) - 45), (int) (sr.getScaledHeight()* (Kabbalah.settingsManager.getSettingByName(this, "Height").getValDouble()/100)), 0xff82F6FF);
			}
			GL11.glPopMatrix();
		}
